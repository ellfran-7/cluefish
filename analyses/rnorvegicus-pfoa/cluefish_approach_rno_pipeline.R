...
# The dataset contains custom item IDs, generated by Sciome. These IDs combine gene names with additional numbers of unknown origin. To ensure proper gene annotation mapping, we need to clean them up. However, we still retain the original IDs since they are used in DRomics for dose-response modeling computations.
...
dr_t_ids <- bg_t_ids_mod[bg_t_ids_mod$transcript_id %in% b_definedCI_onlyids$id_mod,]
