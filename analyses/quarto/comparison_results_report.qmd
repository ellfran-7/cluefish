...
editor: visual
cap-location: margin
output-file: "comparison_results_report_sc09_cf4_2024-05-15.html"
---
...
stand_unfiltered <- stand_res$unfiltered

# the filtered results
stand_filtered <- stand_res$filtered
...
lonely_cluster_analysis_res <- readRDS(here::here("analyses", "lonely_cluster_analysis", "lonely_cluster_analysis_res.rds"))
```
...
lonely_go <- unique((lonely_cluster_analysis_res$unfiltered |> 
                      dplyr::filter(source == "GO:BP"))$term_name)
...
With filters applied to the enriched terms within following steps of the standard and proposed workflow, we can re-examine the discrepancies between enriched terms considered between both workflow.
...
lonely_go <- unique((lonely_cluster_analysis_res$filtered$dr_c_a_lonely |> 
                      dplyr::filter(source == "WP"))$term_name)
...
dr_t_a_stand_ids <- merge(dr_g_a_stand, dr_t_ids, by = "ensembl_gene_id")

cluster_transcripts <- (b_lonely_fishres |> 
  dplyr::filter(new_clustr != "Lonely"))$id

# Retrieve transcript ids for the lonely genes
lonely_ids_data <- merge(lonely_cluster_analysis_res$filtered$dr_g_a_lonely, dr_t_ids, by = "ensembl_gene_id")
...
|----------------------------------|----------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------|
| Number of transcripts considered | `r length(unique(dr_t_a_stand_ids$ensembl_transcript_id_version))`                                       | `r length(unique(total_considered_in_propos))`                                       |
| Number of lonely transcripts     | `r length(unique(BMDres_definedCI$id)) - length(unique(dr_t_a_stand_ids$ensembl_transcript_id_version))` | `r length(unique(BMDres_definedCI$id)) - length(unique(total_considered_in_propos))` |
