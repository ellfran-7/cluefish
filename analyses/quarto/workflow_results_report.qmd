...
## The results as a sensitivity plot
...
# Retrieve the 10 most sensitive clusters and their BMD 25th quantile
...
# Switch "external_gene_name" to "id" column for the workflow output : 
colnames(clustr_df4drplots)[colnames(clustr_df4drplots) == "id"] <- "ensembl_transcript_id_version"
colnames(clustr_df4drplots)[colnames(clustr_df4drplots) == "external_gene_name"] <- "id"

# Create a curvesplot of cluster 1 transcripts with 'cluster' identification and 'transcript count' in title
cp <- DRomics::curvesplot(clustr_df4drplots, addBMD = TRUE, scaling = TRUE, colorby = "trend",
  ...
  geom_vline(xintercept = unique(f$omicdata$dose), linetype = 2, colour = "#302533", alpha = 0.7, size = 0.3) +
...
# Switch ids back to  their original id column for the workflow output : 
colnames(clustr_df4drplots)[colnames(clustr_df4drplots) == "id"] <- "external_gene_name"
colnames(clustr_df4drplots)[colnames(clustr_df4drplots) == "ensembl_transcript_id_version"] <- "id"

clustr_df4drplots_redund <- subset(clustr_df4drplots, select = c(external_gene_name,
                                                                 BMD.zSD,
                                                                 TF,
                                                                 trend))
...
clustr_df4drplots_redund |> 
  ...
  column_spec(2, color = "white", background = spec_color(clustr_df4drplots_redund$BMD.zSD, alpha = 0.8, begin = 0.2, end = 0.9, option = "viridis", direction = -1))
```
