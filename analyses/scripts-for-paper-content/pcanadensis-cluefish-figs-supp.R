...
clustr_enrichres <- readRDS(here::here("outputs", file_date, paste0("clustr_enrichres_", file_date, "_cs06_i2_cf4.rds")))
...
b_lonely_fishres <- merge(lonelyfishing_data$dr_t_c_a_fishing, b_definedCI_mod, by.x = "transcript_id", by.y = "id_mod")
...
venns4paper(
  ...
  filename = here::here("figures", "for-combo", paste0("fig-venndiagramm-1-for-layout-", gsub(":", "", source_vector[1]), "-pcanadensis.png")),
  height = 2.5, 
  width = 2.5, 
  units= "cm",
  resolution = 600,
...
venns4paper(
  ...
  filename = here::here("figures", "for-combo", paste0("fig-venndiagramm-2-for-layout-", gsub(":", "", source_vector[2]), "-pcanadensis.png")),
  height = 5, #4.5 
  width = 5, #4.5
  units= "cm",
  resolution = 900,
...
filename <- paste0("fig-combined-pcanadensis-", Sys.Date())
