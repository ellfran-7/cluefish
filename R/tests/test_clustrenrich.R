...

{
  ...
  if (file.exists(file.path(path, output.filename)) && !overwrite) {
    ...
    # Construct a df for rbind with the gost enrichment output
    ...
    # Now create the resulting list combiining the enrichment results and the annotations
    clustr.enrich <- list(enrichres = full.data,
                          annotations = annot.genes)
    
    # Name the structure of the result
    clustr.enrich <- structure(clustr.enrich, class = "clustrenrich")
    
    return(clustr.enrich)
    
    write.table(clustr.enrich, file.path(path, output.filename))
